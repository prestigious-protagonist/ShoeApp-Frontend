    ...
    const fetchFavourites = async () => {
      ...
      try {
        ...
        res.data.data?.length === 0
          ? toast.warn('No favourites exist.')
  ...
  const handleRemove = async (item) => {
    try {
      ...
      const response = await axios.delete(
        `${import.meta.env.VITE_API_BASE_URL}/productService/api/user/removeFavourite/${item.shoeId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response.data.success) {
        ...
        setFavourites((prev) => prev.filter((fav) => fav.shoeId !== item.shoeId));
